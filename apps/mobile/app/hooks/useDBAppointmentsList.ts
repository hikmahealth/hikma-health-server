 ...
 * @param {Appointment.Status | "all"} status (optional)
 ...
 */
export function useDBAppointmentsList(
  startDate: Date,
  endDate: Date,
  clinicIds: string[] = [],
  status?: Appointment.Status | "all",
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      const fetchedAppointments = await appointmentsCollection
  ...
  return {
    ...
    refresh,
    loadMore,
    appointmentsCount: totalAppointmentsCount,
    loadedAppointmentsCount: appointments.length,
  }
}
